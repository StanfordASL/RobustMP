function [pos_coef,vel_coef,acc_coef,jer_coef,T_seg,coeff] = min_snap(path,obs,fig)
%path: FMT* (or waypoint) path

%smoothing optimization:
%J = p^T Q p 
%s.t. A*p = d = [d_F;d_P]; where d_F: known, d_P: unknown
%set p = A\d <-> J = d^T (A^-T Q A^-1) d 
...
while (~done)
    ...
    for ns = 1:N_seg
        T_seg(ns) = norm(path(ns+1,:)-path(ns,:))/1.5;
    end
    ...
    %Get J
%     d = [d_F;d_P];
%     J = trace(d'*A_inv'*Q_full_fun(T_seg)*A_inv*d);
    coeff = A_inv*[d_F;d_P];
